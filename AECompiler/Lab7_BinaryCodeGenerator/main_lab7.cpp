...
void gen1Mode(const string& inFile, const string& outPortableCode, const string& outSymbolTableFile, bool isOptimizate);

void gen2Mode(const string& inFile, const string& outPostfixExpression, const string& outSymbolTableFile, bool isOptimizate);

void gen3Mode(const string& inFile, const string& outBinaryCode, bool isOptimizate);
...
int main(int argc, char* argv[])
{
  ...
  const string GEN3_MODE = "gen3";

  const string OPTIONAL_OPTIMIZATION_MODE = "opt";
  ...
  const string ERROR_GEN1_MODE = "   - " + GEN1_MODE + " [opt] <файл с входящим выражением> <файл с трехадресным кодом> <файл таблицы символов>\n";
  const string ERROR_GEN2_MODE = "   - " + GEN2_MODE + " [opt] <файл с входящим выражением> <файл с постфиксным выражением> <файл таблицы символов>\n";
  const string ERROR_GEN3_MODE = "   - " + GEN3_MODE + " [opt] <файл с входящим выражением> <файл бинарным трехадресным кодом>\n";
  ...
  try
  {
    ...
    else if (workMode == GEN1_MODE)
    {
      ...
      int offset = 0;
      string possible_opt_mode(argv[2]);
      if (possible_opt_mode == OPTIONAL_OPTIMIZATION_MODE)
        offset = 1;

      string inExpression(argv[2 + offset]);
      string outPortableCode(argv[3 + offset]);
      string outSymbolTable(argv[4 + offset]);

      gen1Mode(inExpression, outPortableCode, outSymbolTable, offset == 1);
    }
    ...
    else if (workMode == GEN2_MODE)
    {
      ...
      string inExpression(argv[2 + offset]);
      string outPostfixExpr(argv[3 + offset]);
      string outSymbolTable(argv[4 + offset]);

      gen2Mode(inExpression, outPostfixExpr, outSymbolTable, offset == 1);
    }

    else if (workMode == GEN3_MODE)
    {
      if (argc < 4)
      {
        cout << ERROR_GEN2_MODE;
      ...
      string inExpression(argv[2 + offset]);
      string outBinaryCode(argv[3 + offset]);

      gen3Mode(inExpression, outBinaryCode, offset == 1);
    }
...
void gen1Mode(const string& inFile, const string& outPortableCode, const string& outSymbolTableFile, bool isOptimizate)
{
  ...
  ThreeAddressCodeGenerator simpleCodeGenerator(symbolTable);
  ThreeAddressCodeGenerator* codeGenerator = &simpleCodeGenerator;

  OptimizeThreeAddressGenerator optimizeCodeGenerator(symbolTable);

  if (isOptimizate)
  {
    ExpressionOptimizator optimizator;

    optimizator.run(modifyTree);

    codeGenerator = &optimizeCodeGenerator;
  }
...
void gen3Mode(const string& inFile, const string& outBinaryCode, bool isOptimizate)
{
  ...
  File outBinaryCodeFile(outBinaryCode, true);

  BinaryCodeGenerator binGenerator(outBinaryCodeFile, symbolTable, instructions);
